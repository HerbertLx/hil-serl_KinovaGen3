...
class KinovaEnv(gym.Env):
    def __init__(
        ...
        self.step_stop = False # True means set v to zero after step, False means the next step will override
        ...
        if not fake_env:
            ...
            def on_press(key):
                ...
                if key == keyboard.Key.cmd_r:
                    self.step_stop = not self.step_stop
                    print(f"🔄 Step Stop Toggled to: {self.step_stop}")
    ...
    def step(self, action: np.ndarray) -> tuple:
        """
        精简版 Step：Action 增量直接修正并下发。
        """
        start_time = time.time()
        
        # 0. 基础限幅 [-1, 1]
        action = np.clip(action, self.action_space.low, self.action_space.high)
        # 1. 安全回弹修正：根据当前位置直接修改 action 里的分量

        # 2. 缩放 Action 到物理量 (Velocity Command)
        # 位置 scale (m/s), 角度 scale (rad/s), 夹爪不需要 scale 直接用
        velocity_cmd = np.zeros(6)
        velocity_cmd[:3] = action[:3] * self.action_scale[0] # XYZ
        velocity_cmd[3:6] = action[3:6] * self.action_scale[1] # RPY (弧度)

        velocity_cmd[:6] = self.clip_action_by_safety(velocity_cmd[:6].copy())
        # 4. 硬件指令下发
        # 4.1 机械臂速度
        ...
        # 6. 周期结束停止 (防止滑动)
        if self.step_stop:
            self._send_velocity_command(np.zeros(6))
    ...
    # 辅助函数
    # ---------------------------------------------------------
    def clip_action_by_safety(self, action: np.ndarray) -> np.ndarray:
        """
        根据当前位姿，修正 6 维 Action 增量。
        输出优化：使用直观方向描述，多重越界合并显示。
        """
        curr_pose = self.currpos.copy()
        warnings = []  # 用于收集所有的警告描述

        # 1. 位置判定 (XYZ) 映射表
        # Axis 0: 前(+)/后(-), Axis 1: 左(+)/右(-), Axis 2: 上(+)/下(-)
        pos_desc = {
            0: {"low": "太后", "high": "太前"},
            1: {"low": "太右", "high": "太左"},
            2: {"low": "太下", "high": "太上"}
        ...
        for i in range(3):
            if curr_pose[i] <= self.xyz_bounding_box.low[i]:
                if action[i] <= 0:
                    action[i] = 0.1
                    warnings.append(pos_desc[i]["low"])
            elif curr_pose[i] >= self.xyz_bounding_box.high[i]:
                if action[i] >= 0:
                    action[i] = -0.1
                    warnings.append(pos_desc[i]["high"])

        # 2. 角度判定 (RPY)
        curr_euler = quat_2_euler(curr_pose[3:]) 
        while curr_euler[0] < 0:
            curr_euler[0] += 2 * np.pi
                
        euler_bounce_speed = 2.0  

        # Rot 0 (Roll) -> 对应 action[4]
        if curr_euler[0] < self.rpy_bounding_box.low[0]:
            if action[4] <= 0:
                action[4] = euler_bounce_speed
                warnings.append("后仰幅度过大")
        elif curr_euler[0] > self.rpy_bounding_box.high[0]:
            if action[4] >= 0:
                action[4] = -euler_bounce_speed
                warnings.append("前倾幅度过大")

        # Rot 1 (Pitch) -> 对应 action[3]
        if curr_euler[1] < self.rpy_bounding_box.low[1]:
            if action[3] >= 0:
                action[3] = -euler_bounce_speed
                warnings.append("右翻幅度过大")
        elif curr_euler[1] > self.rpy_bounding_box.high[1]:
            if action[3] <= 0:
                action[3] = euler_bounce_speed
                warnings.append("左翻幅度过大")

        # Rot 2 (Yaw) -> 对应 action[5]
        if curr_euler[2] < self.rpy_bounding_box.low[2]:
            if action[5] <= 0:
                action[5] = euler_bounce_speed
                warnings.append("顺时针旋转过大")
        elif curr_euler[2] > self.rpy_bounding_box.high[2]:
            if action[5] >= 0:
                action[5] = -euler_bounce_speed
                warnings.append("逆时针旋转过大")

        # --- 统一打印逻辑 ---
        if warnings:
            # 将多个警告用 "|" 分隔，合并在同一行
            print(f"  ⚠️  安全限制触发: {' | '.join(warnings)}")
    ...
    def get_im(self) -> Dict[str, np.ndarray]:
        ...
        full_res_images = {}  # 新增：用于存储未缩放的高清裁剪图
        ...
        for key, cap in self.cap.items():
            try:
                ...
                # 3. 填充各个图像字典
                images[key] = resized[..., ::-1].copy()  # 转换为 RGB 供 Agent 使用
                display_images[key] = resized.copy()     # BGR 供可视化
                display_images[key + "_full"] = cropped.copy()
        ...
        if self.display_image and display_images:
            try: 
                # 使用 put_nowait 或 put 取决于您对实时性的要求，此处保留原逻辑
    ...
    def _send_velocity_command(self, vel: np.ndarray):
        """
        内部逻辑：将 6 维速度指令直接通过 KinovaManager 发送。
        """
        # 直接调用原来的核心函数
        # print(f"\nIn _send_velocity_command: sending velocity command to robot: {vel}")
        self.robot.move_velocity(vel)
        ...
        try:
            if len(self.recording_frames):
                ...
                for camera_key in self.recording_frames[0].keys():
                    ...
                    video_path = f'./videos/video_{camera_key}_{timestamp}.mp4'
...
def test_movement():
    ...
    try:
        env = KinovaEnv(hz=10, config=TestConfig(), fake_env=False)
        env.reset()
        print(f"Env Reset Complete.")
        
        # 向上移动 (Z+)
        gripper = 1.0 # 1 is open the gripper, while -1 is close the gripper
        for i in range(1):

            action = np.array([0.0, 0.0, 0.5, 0.0, 0.0, 0.0, gripper]) # Gripper close
